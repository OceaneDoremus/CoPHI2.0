...
export function Plot() {
  ...
  function ribbonPathString(source, target, tension) {
    ...
    return tension === 1
      ? `M${sx},${source.y0}L${tx},${target.y0}v${tdy}L${sx},${
          source.y0 + sdy
        }Z`
      : `M${sx},${source.y0}C${tension * sx + (1 - tension) * tx},${
          source.y0
        } ${tension * tx + (1 - tension) * sx},${target.y0} ${tx},${
          target.y0
        }v${tdy}C${tension * tx + (1 - tension) * sx},${target.y0 + tdy} ${
          tension * sx + (1 - tension) * tx
        },${source.y0 + sdy} ${sx},${source.y0 + sdy}Z`;
  }
  /**
  ...
  function pcp(selection) {
    ...
    const dragging = {};
    allAxes.call(
      d3.drag().on("start", dragStart).on("drag", dragMoved).on("end", dragEnd)
    );
    ...
    const checkboxGroup = allAxes
      ...
      .on("click", function (event, d) {
        const elementIndex = dimensions.findIndex(
          (element) => element === d.name
        );
        let currentEl = d3.selectAll(".axis").filter(function () {
          return d3.select(this).attr("name") == d.name;
        });
        currentEl.attr("visibility", "hidden");
        let newAxes = axes.splice(
          axes.findIndex((axe) => axe.name == d.name),
          1
        );
    ...
    var gradients = {
      gradient2: createLinearGradient(
        "gradient2",
        "0%",
        "10%",
        "0%",
      ...
      gradient1: createLinearGradient(
        "gradient1",
        "0%",
        "0%",
        "0%",
      ...
      gradient0: createLinearGradient(
        "gradient0",
        "0%",
        "0%",
        "0%",
    ...
    allAxes.each(function (axis) {
      rect = d3
        ...
        .attr(
          "fill",
          (d) => "url(#" + gradients["gradient" + d.value].attr("id") + ")"
        )
        .on("mouseover", function (event, d) {
          let info =
            "<strong>Name</strong> : " +
            ...
            d.count +
            "<strong><br>Percentage</strong> : " +
            d.percentage +
            " %" +
            "<strong><br>Category</strong> : " +
            d.value +
            "<strong><br>Axe</strong> : " +
            d.parentAxe +
          ...
          d3.selectAll(".rect-nodes")
            .filter(
              (_, i, nodes) =>
                d3.select(nodes[i]).attr("parent") === d.parentAxe
            )
            .attr("opacity", 0.5);
        })
        .on("mouseout", function () {
          hideTip();
          d3.selectAll(".rect-nodes").attr("opacity", 1);
        });
    });
    rect = d3.selectAll(".rect-nodes").on("click", function (event, d) {
      ...
      const childFilter = child.filter((el) =>
        graph.includes(el.getAttribute("graphPosition"))
      );
      const parents = findAncestors(g, el);
      const parentsFilter = parents.filter((el) =>
        graph.includes(el.getAttribute("graphPosition"))
      );
      coloredPaths = coloredPaths.concat(current, childFilter, parentsFilter);
      d3.selectAll(coloredPaths).attr("fill", colorsP[1]);
    });
  }
  ...
  pcp.render = function (v, selection) {
    ...
    axes.push(v);
    var filteredElements = d3
      .selectAll('g.axis[name="' + v.name + '"]')
  ...
  pcp.transition = function (selection) {
    ...
    ax.attr("transform", function (d) {
      return "translate(" + xScale(d.name) + ")";
    });
    const paths = d3.selectAll("path").attr("transform", function (d) {
      return "translate(" + xScale(d.name) + ")";
    });
    return pcp;
  };
  ...
  pcp.xScale = function (range, domain) {
    ...
    xScale = d3.scalePoint().range([0, range]).padding(1).domain(domain);
    return pcp;
  };
  ...
  function updateOpacity(selection) {
    d3.selectAll(".rect-nodes").style("opacity", opacity.nodes);
    d3.selectAll("path").style("opacity", opacity.paths);
  }
  function updateNodesColors() {
    d3.selectAll(".rect-nodes")
      .style("fill", function (d) {
        return colorsN[d.value];
      })
      .attr("opacity", 1);
  }
